...
def train(fold, model_name):
    ...
    columns_to_train = ["id", "price", "minimum_nights", "number_of_reviews", "noise(dB)", "kfold"]
    ...
    X_train = df_train[[col for col in df_train.columns if col not in ['kfold', "price"]]]
    X_val = df_valid[[col for col in df_valid.columns if col not in ['kfold', 'price']]]
