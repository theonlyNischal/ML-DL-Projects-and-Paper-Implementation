...
def train(fold, model_name):
    ...
    columns_to_train = ["building_id", "damage_grade", "geo_level_1_id", "geo_level_2_id", "geo_level_3_id", "count_floors_pre_eq", "age", "area_percentage", "height_percentage", "has_superstructure_adobe_mud", "has_superstructure_mud_mortar_stone", "has_superstructure_stone_flag", "has_superstructure_cement_mortar_stone", "has_superstructure_mud_mortar_brick", "has_superstructure_cement_mortar_brick", "has_superstructure_timber", "has_superstructure_bamboo", "has_superstructure_rc_non_engineered", "has_superstructure_rc_engineered", "has_secondary_use_agriculture", "has_secondary_use_hotel", "has_secondary_use_rental", "has_secondary_use_institution", "kfold"]

    df = df[columns_to_train]
    # Meta features
    ...
    X_train = df_train[[col for col in df_train.columns if col not in ['kfold', 'damage_grade']]]


    X_val = df_valid[[col for col in df_valid.columns if col not in ['kfold', 'damage_grade']]]

    # Convert the labels into numpy array
    y_train = df_train["damage_grade"].values
    y_valid = df_valid["damage_grade"].values
